...
(function(){
    ...
    var transitionEndEvent = transitionEnd().end;
	...
	Y.fn._move = function(e){
		...
		if(this.opts.nesting){
			if( Math.abs(abs_disX - abs_disY) > 5 && !this._dis ){
				this._dis = abs_disX > abs_disY ? -1 : 1;
			}else{
				return;
			}

			if(this._dis == -1 && this.opts.hScroll){
				e.stopPropagation();
			}

			if(this._dis == 1 && this.opts.vScroll){
				e.stopPropagation();
			}
		}
		...
		e.preventDefault();
		if(!this.opts.bounce){
			var temp = this._bounce(newX, newY);
			newX = temp[0];
			newY = temp[1];
		}
	...
	Y.fn._end = function(e){
		...
		if(this.opts.nesting && !this._dis){
			this._dis = 0;
		}
